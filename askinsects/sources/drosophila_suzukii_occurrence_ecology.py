...
DROSOPHILA_SUZUKII_OCCURRENCE_ECOLOGY_SOURCE_ID = "drosophila_suzukii_occurrence_ecology"
...
class DrosophilaSuzukiiObservation:
    record_id: str
    title: str
    text: str
    url: str | None
...
@dataclass(frozen=True)
class DrosophilaSuzukiiOccurrenceEcologyResult:
    source_id: str
...
def _country_from_place(place: object) -> str | None:
    if not isinstance(place, str) or not place.strip():
        return None
    if "台灣" in place or "臺灣" in place or "Taiwan" in place:
        return "Taiwan"
    parts = [part.strip() for part in place.split(",") if part.strip()]
    if not parts:
        return None
    if len(parts) == 1:
        normalized = _normalize_country(parts[0])
        known_single_values = {
            "Australia",
            "Austria",
            "Belgium",
        ...
        return normalized if normalized in known_single_values else None
...
def _normalize_country(value: str | None) -> str | None:
    ...
    return normalized if normalized in known_countries else normalized


def _habitat_values(raw_observation: dict[str, Any], raw_occurrence: dict[str, Any] | None = None) -> tuple[str, ...]:
    values: list[str] = []
    ...
    if isinstance(ofvs, list):
        for field in ofvs:
            ...
            if value:
                values.append(value)
    if raw_occurrence:
        for key in ("habitat", "occurrenceRemarks", "verbatimLocality"):
            value = str(raw_occurrence.get(key) or "").strip()
            if value and key == "habitat":
                values.append(value)
    return tuple(dict.fromkeys(values))


def _observation_from_row(row: sqlite3.Row) -> DrosophilaSuzukiiObservation | None:
    ...
    if upstream_source == "gbif":
        raw = payload.get("raw_occurrence")
        if not isinstance(raw, dict):
            return None
        country = _normalize_country(str(raw.get("country") or "").strip() or None)
        observed_date = _date_prefix(raw.get("eventDate") or raw.get("dateIdentified"))
        month = _month_from_date(observed_date)
        if month is None:
            month = int(raw["month"]) if str(raw.get("month") or "").isdigit() else None
        ...
        quality_grade = str(raw.get("occurrenceStatus") or "").strip() or None
        habitats = _habitat_values({}, raw)
    elif upstream_source == "inaturalist":
    ...
    return DrosophilaSuzukiiObservation(
...
def _observation_rows(conn: sqlite3.Connection) -> list[DrosophilaSuzukiiObservation]:
    rows = conn.execute(
        """
        ...
        """,
        (DROSOPHILA_SUZUKII_SOURCE_ID, DROSOPHILA_SUZUKII_SPECIES),
    ).fetchall()
    observations: list[DrosophilaSuzukiiObservation] = []
    for row in rows:
...
def _sample_values(values: list[str | None], *, limit: int = 8) -> list[str]:
    return list(dict.fromkeys(value for value in values if value))[:limit]


def _date_range(observations: list[DrosophilaSuzukiiObservation]) -> tuple[str | None, str | None]:
...
def _bbox(observations: list[DrosophilaSuzukiiObservation]) -> dict[str, float] | None:
    ...
    *,
    ...
    payload = {
        **payload,
        ...
        "input_source_counts": dict(sorted(Counter(obs.upstream_source for obs in observations).items())),
    ...
    return EvidenceRecord(
        ...
        provenance=Provenance(
            ...
            license="derived from indexed public GBIF and iNaturalist Drosophila suzukii observation records",
...
def _country_records(observations: list[DrosophilaSuzukiiObservation], *, retrieved_at: str) -> list[EvidenceRecord]:
    records: list[EvidenceRecord] = []
    by_country: dict[str, list[DrosophilaSuzukiiObservation]] = {}
    ...
    for country, country_observations in sorted(by_country.items(), key=lambda item: (-len(item[1]), item[0])):
        ...
        source_counts = Counter(obs.upstream_source for obs in country_observations)
        months = sorted({obs.month for obs in country_observations if obs.month})
        month_names = [MONTH_NAMES[month] for month in months]
        text = (
            f"Drosophila suzukii ({DROSOPHILA_SUZUKII_COMMON_NAME}) occurrence ecology country summary for {country}. "
            f"{len(country_observations)} indexed observation record(s) across {len(source_counts)} upstream source(s): "
        ...
        records.append(
            _record(
                record_id=f"swd_occurrence_ecology:country:{_normalize_id(country)}",
                title=f"Drosophila suzukii occurrence ecology in {country}",
                text=text,
                locator=f"source_index.sqlite#swd-observation-ecology/country/{_normalize_id(country)}",
...
def _country_month_records(observations: list[DrosophilaSuzukiiObservation], *, retrieved_at: str) -> list[EvidenceRecord]:
    records: list[EvidenceRecord] = []
    by_key: dict[tuple[str, int], list[DrosophilaSuzukiiObservation]] = {}
    for observation in observations:
    ...
    for (country, month), month_observations in sorted(by_key.items(), key=lambda item: (item[0][0], item[0][1])):
        ...
        source_counts = Counter(obs.upstream_source for obs in month_observations)
        text = (
            f"Drosophila suzukii ({DROSOPHILA_SUZUKII_COMMON_NAME}) occurrence ecology country-month seasonality record for {country} in {month_name}. "
        ...
        records.append(
            _record(
                record_id=f"swd_occurrence_ecology:country_month:{_normalize_id(country)}:{month:02d}",
                title=f"Drosophila suzukii seasonality in {country}: {month_name}",
                text=text,
                locator=f"source_index.sqlite#swd-observation-ecology/country-month/{_normalize_id(country)}/{month:02d}",
...
def _habitat_records(observations: list[DrosophilaSuzukiiObservation], *, retrieved_at: str) -> list[EvidenceRecord]:
    records: list[EvidenceRecord] = []
    by_key: dict[tuple[str, str], list[DrosophilaSuzukiiObservation]] = {}
    for observation in observations:
    ...
    for (country, habitat), habitat_observations in sorted(by_key.items(), key=lambda item: (-len(item[1]), item[0][0], item[0][1])):
        text = (
            f"Drosophila suzukii ({DROSOPHILA_SUZUKII_COMMON_NAME}) occurrence ecology habitat annotation for {country}. "
            f"Habitat value: {habitat}. "
            f"{len(habitat_observations)} indexed observation record(s) include this public habitat-like field value."
        )
        records.append(
            _record(
                record_id=f"swd_occurrence_ecology:habitat:{_normalize_id(country)}:{_normalize_id(habitat)}",
                title=f"Drosophila suzukii habitat annotation in {country}: {habitat}",
                text=text,
                locator=f"source_index.sqlite#swd-observation-ecology/habitat/{_normalize_id(country)}/{_normalize_id(habitat)}",
                observations=habitat_observations,
                payload={
                    "aggregation_type": "habitat_summary",
                    "country": country,
                    "habitat": habitat,
                    "habitat_source": "public observation habitat-like metadata",
                },
                retrieved_at=retrieved_at,
            )
        )
...
def build_drosophila_suzukii_occurrence_ecology_records(
    artifact_dir: Path,
    *,
    retrieved_at: str | None = None,
) -> DrosophilaSuzukiiOccurrenceEcologyResult:
    ...
    country_month_count = len(records) - country_count
    records.extend(_habitat_records(observations, retrieved_at=retrieved))
    habitat_count = len(records) - country_count - country_month_count
        ...
        gaps.append(
            {
                "source": DROSOPHILA_SUZUKII_OCCURRENCE_ECOLOGY_SOURCE_ID,
                "lane": "ecology",
                "reason": "no_indexed_drosophila_suzukii_observation_records",
                "input_source": DROSOPHILA_SUZUKII_SOURCE_ID,
                "retrieved_at": retrieved,
            }
        )

    return DrosophilaSuzukiiOccurrenceEcologyResult(
        source_id=DROSOPHILA_SUZUKII_OCCURRENCE_ECOLOGY_SOURCE_ID,
        records=records,
        gaps=gaps,
        observation_count=len(observations),
