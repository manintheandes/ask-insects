...
DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID = "drosophila_suzukii_olfaction_literature"
...
class DrosophilaSuzukiiOlfactionLiteratureResult:
    source_id: str
...
def _record_for_article(
    *,
    ...
    payload = {
        ...
        "query": PUBMED_QUERY,
        "scope": f"{SPECIES} olfaction/chemosensation literature from 2010 onward",
        "primary_taxon": SPECIES,
        "common_name": COMMON_NAME,
    }
    text_parts = [
        title,
        f"{SPECIES} ({COMMON_NAME}) PubMed olfaction/chemosensation literature audit candidate since 2010.",
    ...
    return EvidenceRecord(
        record_id=f"swd_olfaction_literature:pubmed:{_safe_id(pmid)}",
        lane="literature",
...
def fetch_drosophila_suzukii_olfaction_literature_records(
    *,
    raw_dir: Path,
...
) -> DrosophilaSuzukiiOlfactionLiteratureResult:
    ...
    return DrosophilaSuzukiiOlfactionLiteratureResult(
        source_id=DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID,
        records=records,
        gaps=gaps,
