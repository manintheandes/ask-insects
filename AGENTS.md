...
For a normal user-facing insect question, the route is already decided. Do not send commentary, a status update, or a routing preamble. The turn must contain exactly one visible assistant message: the command's complete stdout as the final answer. Do not spend a reasoning turn deciding how to route it. As the first action, run the preferred first and only operational command: `ask-insects ask 'USER QUESTION' --answer-only`. Replace `USER QUESTION` with the user's exact complete question as one POSIX single-quoted shell argument. Inside that argument, replace every embedded apostrophe with the exact shell sequence `'"'"'`. Never put the user's question inside double quotes, command substitution, or an unquoted shell word. Compose the safe command before calling the tool. Never retry after a shell-quoting error; that is a failed route, not permission for a second command. If the first command yields a session ID, continue the same process with `write_stdin` until it exits and return the accumulated output; this is continuation of the one command, not a second command. Do not load the installed skill; its description and this file already provide the complete route. No other command may precede or follow the hosted call. Never call image generation, `imagegen`, `justsee`, visualization, screenshot, or any other visual helper unless the user explicitly asks for a visual. Do not inspect memory, Chronicle, repository docs, other skills, or any other file first. The command's stdout is already the complete sourced answer. Return it immediately and verbatim without another reasoning turn or command. Do not parse, rewrite, preface, or append to it. Preserve canonical labels such as `SWD crop repellent` and `Human mosquito repellent`, plus every source ID and locator exactly as returned.
