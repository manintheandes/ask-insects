...
This writes official GFF, annotated protein FASTA, annotated CDS FASTA, annotated transcript FASTA, GO GAF, codon usage, identifier event history, current-ID resolution rows, NCBI LinkOut, and OrthoMCL CURRENT corePairs ortholog, coortholog, and inparalog downloads under `artifacts/mosquito-v1/raw/vectorbase_genomics/`, normalizes records into `genes`, `transcripts`, `proteins`, and `genome_features`, stores parsed payloads in SQLite, and keeps provenance to the saved file line, FASTA header, or LinkOut entry. OrthoMCL pair records are parsed only when either side starts with the old Aedes namespace prefix `aaeg-old|AAEL`; they are current-release pair records in the old AAEL namespace, not orthogroups. That remaining advanced-orthology boundary is also indexed as a queryable source-gap record, so those questions return genomics evidence instead of drifting into unrelated media. FASTA sequence records store sequence metadata and observed lengths, not every nucleotide as answer text.
...
```bash
python3 -m askinsects ingest-video-atoms --mirror-videos --generate-artifacts --discover-sources --max-video-bytes 750000000 --allowed-licenses "CC0,CC-BY,CC BY,Creative Commons,https://spdx.org/licenses/CC0-1.0.html"
...
The lane derives from PMC, Dryad, Mendeley, OSF, Zenodo, Figshare, and repository-discovery candidates. For each downloadable video it stores checksum, byte size, duration, fps, resolution, codec, source paper or dataset, license, and exact locator when mirroring and probing are allowed. If the file is too large, missing a download URL, blocked by license uncertainty, an archive that has not been member-expanded, cannot be probed, is not actually video, comes from a repository sweep with no usable candidates, or appears only as a Zenodo/Figshare manifest gap, the ingest writes a queryable `video_gap` record instead of pretending coverage exists. Gap records preserve the source download URL, source URL, byte size, source-provided hashes when available, license text, source dataset, repository, original source, original reason, and locator. Discovery scope is strict: repository search terms do not count as Aedes evidence unless the source title, description, file name, citation, species field, or equivalent material metadata names `Aedes aegypti`. When artifact generation is enabled it emits thumbnails, keyframes, preview clips, and frame manifests under `raw/video_atoms/`. Motion table inputs become `behavior` rows with behavior type, life stage, sex, assay, stimulus, arena, frame/time, track ID, coordinates, and confidence when present. `--discover-sources` runs bounded PMC OA, Dryad, Mendeley, OSF, Zenodo, Figshare, institutional Dataverse-style, and indexed paper-supplement discovery. Each repository also gets a `video_sweep` receipt record plus a receipt entry with status, raw candidate count, accepted candidate count, gap count, and limit status; the completion gate requires these sweep receipts for all eight targets. The May 25, 2026 hosted video-atoms refresh installed 46,020 Aedes video-atom records: 91 video assets, 21 verified mirrored videos, 84 inspectable artifacts, 45,574 motion rows, and 271 structured video gaps.
