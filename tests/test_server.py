...
class ServerTests(unittest.TestCase):
    ...
    def test_ingest_drosophila_suzukii_osu_trap_reports_route_passes_options(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            artifact_dir = Path(tmpdir) / "mosquito-v1"
            build_fixture_index(artifact_dir=artifact_dir)
            with mock.patch(
                "scripts.ingest_drosophila_suzukii_osu_trap_reports.ingest_drosophila_suzukii_osu_trap_reports"
            ) as ingest:
                ingest.return_value = {
                    "ok": True,
            ...
            self.assertEqual(response.status, 200)
            self.assertTrue(response.payload["ok"])
            self.assertEqual(response.payload["source"], "drosophila_suzukii_osu_trap_reports")
            self.assertEqual(response.payload["activated_artifact_dir"], str(artifact_dir))
            ingest.assert_called_once_with(
                artifact_dir=artifact_dir,
                retrieved_at="2026-05-30T00:00:00Z",
    ...
    def test_ingest_drosophila_suzukii_dryad_landscape_monitoring_route_passes_options(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            artifact_dir = Path(tmpdir) / "mosquito-v1"
            build_fixture_index(artifact_dir=artifact_dir)
            with mock.patch(
                "scripts.ingest_drosophila_suzukii_dryad_landscape_monitoring.ingest_drosophila_suzukii_dryad_landscape_monitoring"
            ) as ingest:
                ingest.return_value = {
                    "ok": True,
            ...
            self.assertEqual(response.status, 200)
            self.assertTrue(response.payload["ok"])
            self.assertEqual(response.payload["source"], "drosophila_suzukii_dryad_landscape_monitoring")
            self.assertEqual(response.payload["activated_artifact_dir"], str(artifact_dir))
            ingest.assert_called_once_with(
                artifact_dir=artifact_dir,
                retrieved_at="2026-05-30T00:00:00Z",
    ...
    def test_ingest_drosophila_suzukii_plos_climate_suitability_route_passes_options(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            artifact_dir = Path(tmpdir) / "mosquito-v1"
            build_fixture_index(artifact_dir=artifact_dir)
            with mock.patch(
                "scripts.ingest_drosophila_suzukii_plos_climate_suitability.ingest_drosophila_suzukii_plos_climate_suitability"
            ) as ingest:
                ingest.return_value = {
                    "ok": True,
            ...
            self.assertEqual(response.status, 200)
            self.assertTrue(response.payload["ok"])
            self.assertEqual(response.payload["source"], "drosophila_suzukii_plos_climate_suitability")
            self.assertEqual(response.payload["activated_artifact_dir"], str(artifact_dir))
            ingest.assert_called_once_with(
                artifact_dir=artifact_dir,
                retrieved_at="2026-05-30T00:00:00Z",
    ...
    def test_ingest_drosophila_suzukii_umn_flight_assay_rows_route_passes_options(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            artifact_dir = Path(tmpdir) / "mosquito-v1"
            build_fixture_index(artifact_dir=artifact_dir)
            with mock.patch(
                "scripts.ingest_drosophila_suzukii_umn_flight_assay_rows.ingest_drosophila_suzukii_umn_flight_assay_rows"
            ) as ingest:
                ingest.return_value = {
                    "ok": True,
                    "source": "drosophila_suzukii_umn_flight_assay_rows",
                    "record_count": 403,
                    "parsed_row_count": 401,
                }
                response = dispatch_request(
                    "POST",
            ...
            self.assertEqual(response.status, 200)
            self.assertTrue(response.payload["ok"])
            self.assertEqual(response.payload["source"], "drosophila_suzukii_umn_flight_assay_rows")
            self.assertEqual(response.payload["activated_artifact_dir"], str(artifact_dir))
            ingest.assert_called_once_with(
                artifact_dir=artifact_dir,
                max_download_bytes=123456,
