...
class IngestZenodoAedesVideosTests(unittest.TestCase):
    def test_ingest_updates_existing_artifact_without_removing_other_sources(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(counts[("zenodo_aedes_videos", "media")], 2)
            payload_rows = SourceIndex(artifact_dir / "source_index.sqlite").sql(
                "select count(*) as n from record_payloads where source='zenodo_aedes_videos'"
            )
            self.assertEqual(payload_rows[0]["n"], 2)
            gap_rows = SourceIndex(artifact_dir / "source_index.sqlite").sql(
                "select count(*) as n from record_payloads where source='zenodo_aedes_videos' and payload_json like '%\"atom_type\": \"video_gap\"%'"
            )
