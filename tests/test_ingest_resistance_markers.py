...
class IngestResistanceMarkersTests(unittest.TestCase):
    def test_ingest_updates_existing_artifact_without_removing_other_sources(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertGreaterEqual(counts[("aedes_resistance_markers", "resistance")], 3)
            self.assertEqual(counts[("mosquito_v1_fixtures", "taxonomy")], 1)
            payload_rows = index.sql(
                "select count(*) as n from record_payloads where source='aedes_resistance_markers'"
            )
            self.assertGreaterEqual(payload_rows[0]["n"], 3)
            status = json.loads((artifact_dir / "source_status.json").read_text(encoding="utf-8"))
            self.assertIn("aedes_resistance_markers", status["sources"])
            self.assertEqual(status["aedes_resistance_markers"]["record_count"], result["record_count"])
