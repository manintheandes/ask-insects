...
class AgentSetupTests(unittest.TestCase):
    ...
    def test_repo_instructions_make_the_hosted_call_the_first_normal_action(self):
        ...
        for term in (
            ...
            "write_stdin",
            "The invocation line must begin with `ask-insects`",
            "literal here-document",
            "--question-stdin",
            "Do not quote, escape, rewrite, or interpolate the question body",
            "inside its single `<input>...</input>` element",
            "Never copy `<codex_delegation>`",
            "`<source_thread_id>`",
