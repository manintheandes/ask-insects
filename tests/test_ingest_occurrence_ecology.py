...
class IngestOccurrenceEcologyTests(unittest.TestCase):
    def test_ingest_updates_existing_artifact_without_removing_other_sources(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(counts[("aedes_occurrence_ecology", "ecology")], 7)
            self.assertEqual(counts[("mosquito_v1_fixtures", "taxonomy")], 1)
            payload_rows = index.sql(
                "select count(*) as n from record_payloads where source='aedes_occurrence_ecology'"
            )
            self.assertEqual(payload_rows[0]["n"], 7)
            status = json.loads((artifact_dir / "source_status.json").read_text(encoding="utf-8"))
            self.assertIn("aedes_occurrence_ecology", status["sources"])
            self.assertEqual(status["aedes_occurrence_ecology"]["observation_count"], 4)
