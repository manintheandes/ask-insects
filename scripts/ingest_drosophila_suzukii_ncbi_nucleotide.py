...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_NCBI_NUCLEOTIDE_SOURCE_ID)
    ]
...
def _existing_barcode_rows(index: SourceIndex) -> list[dict[str, object]]:
    with index.connect() as conn:
        rows = conn.execute(
            """
            select
            ...
            where r.lane = 'dna_barcodes'
              and r.source in ('drosophila_suzukii_core', 'bold_api')
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    source_payload = {
        ...
        "existing_barcode_row_count": result.existing_barcode_row_count,
        "bold_accession_matched_count": result.bold_accession_matched_count,
        "genbank_only_count": result.genbank_only_count,
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_NCBI_NUCLEOTIDE_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_NCBI_NUCLEOTIDE_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_NCBI_NUCLEOTIDE_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_NCBI_NUCLEOTIDE_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_ncbi_nucleotide(
    *,
    ...
    index = SourceIndex(artifact_dir / "source_index.sqlite")
    index.initialize()
    existing_rows = _existing_barcode_rows(index)
    result = fetch_drosophila_suzukii_ncbi_nucleotide_records(
        raw_dir=artifact_dir / "raw" / "drosophila_suzukii_ncbi_nucleotide",
        existing_barcode_rows=existing_rows,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest a GenBank/NCBI nucleotide cross-check lane for Drosophila suzukii.")
