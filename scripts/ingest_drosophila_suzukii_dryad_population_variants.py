...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_DRYAD_POPULATION_VARIANTS_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_DRYAD_POPULATION_VARIANTS_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_DRYAD_POPULATION_VARIANTS_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_DRYAD_POPULATION_VARIANTS_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_DRYAD_POPULATION_VARIANTS_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_dryad_population_variants(
    *,
    ...
    result = fetch_drosophila_suzukii_dryad_population_variants_records(
        raw_dir=artifact_dir / "raw" / DROSOPHILA_SUZUKII_DRYAD_POPULATION_VARIANTS_SOURCE_ID,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest Drosophila suzukii Dryad population VCF manifests and source gaps.")
    parser.add_argument("--artifact-dir", default=str(DEFAULT_ARTIFACT_DIR))
    parser.add_argument("--max-mirror-bytes", type=int, default=1_000_000_000)
