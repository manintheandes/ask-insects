...
from askinsects.sources.vectorbyte_traits import DEFAULT_QUERY, MAX_SEARCH_LIMIT, VECTORBYTE_TRAITS_SOURCE_ID, fetch_vectorbyte_trait_records
...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    if not isinstance(existing, list):
        existing = []
    combined = [gap for gap in existing if not (isinstance(gap, dict) and gap.get("source") == VECTORBYTE_TRAITS_SOURCE_ID)]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[VECTORBYTE_TRAITS_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if VECTORBYTE_TRAITS_SOURCE_ID not in sources:
                sources.append(VECTORBYTE_TRAITS_SOURCE_ID)
        ...
        payload[VECTORBYTE_TRAITS_SOURCE_ID] = source_payload
...
def ingest_vectorbyte_traits(
    *,
    ...
    result = fetch_vectorbyte_trait_records(
        raw_dir=artifact_dir / "raw" / "vectorbyte_traits",
    ...
    outcome = run_source_ingest(
        ...
        persist_gap_records=False,  # gaps are plain dicts; non-gap trait records always present on success
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest Aedes aegypti VectorByte VecTraits rows into Ask Insects.")
