...
FATAL_REFRESH_GAP_REASONS = {"swd_ncbi_gene_orthologs_download_failed"}
...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_NCBI_GENE_ORTHOLOGS_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    source_payload = {
        ...
        "swd_gene_count": result.swd_gene_count,
        "partner_taxon_count": result.partner_taxon_count,
        "relationship_counts": result.relationship_counts,
        "matched_gene_record_count": result.matched_gene_record_count,
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_NCBI_GENE_ORTHOLOGS_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_NCBI_GENE_ORTHOLOGS_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_NCBI_GENE_ORTHOLOGS_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_NCBI_GENE_ORTHOLOGS_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_ncbi_gene_orthologs(
    *,
    ...
    if not refresh_failed:
        index.replace_source_records(DROSOPHILA_SUZUKII_NCBI_GENE_ORTHOLOGS_SOURCE_ID, result.records)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest NCBI Gene ortholog pairs for Drosophila suzukii.")
    parser.add_argument("--artifact-dir", default=str(DEFAULT_ARTIFACT_DIR))
    parser.add_argument("--max-download-bytes", type=int, default=200_000_000)
