...
from askinsects.sources.anopheles_primary_evidence import (
    ANOPHELES_PRIMARY_EVIDENCE_SOURCE_ID,
    build_anopheles_primary_evidence_records,
)


def ingest_anopheles_primary_evidence(
    *,
    artifact_dir: Path = DEFAULT_ARTIFACT_DIR,
    retrieved_at: str | None = None,
) -> dict[str, object]:
    retrieved = retrieved_at or utc_now()
    records = build_anopheles_primary_evidence_records(retrieved_at=retrieved)
    ...
    source_payload = {
        ...
        "method": "human-reviewed exact Anopheles primary studies",
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Ingest exact primary Anopheles evidence."
    )
    parser.add_argument("--artifact-dir", default=str(DEFAULT_ARTIFACT_DIR))
    parser.add_argument("--retrieved-at")
    args = parser.parse_args(argv)
    result = ingest_anopheles_primary_evidence(
