...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_EXTENSION_GUIDANCE_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True) -> dict[str, object]:
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_EXTENSION_GUIDANCE_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_EXTENSION_GUIDANCE_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_EXTENSION_GUIDANCE_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_EXTENSION_GUIDANCE_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_extension_guidance(
    *,
    ...
    sources = list(DEFAULT_EXTENSION_GUIDANCE_SOURCES)
    if source_urls:
        sources = [
            {
                "organization": "custom",
                "url": url,
                "topic": "spotted wing drosophila extension/IPM guidance",
                "region": "custom",
            }
    ...
    result = fetch_drosophila_suzukii_extension_guidance_records(
        sources,
        raw_dir=artifact_dir / "raw" / "drosophila_suzukii_extension_guidance",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest Drosophila suzukii extension/IPM guidance into Ask Insects.")
