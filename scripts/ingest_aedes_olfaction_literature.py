...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == AEDES_OLFACTION_LITERATURE_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[AEDES_OLFACTION_LITERATURE_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if AEDES_OLFACTION_LITERATURE_SOURCE_ID not in sources:
                sources.append(AEDES_OLFACTION_LITERATURE_SOURCE_ID)
        ...
        payload[AEDES_OLFACTION_LITERATURE_SOURCE_ID] = source_payload
...
def ingest_aedes_olfaction_literature(
    *,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest a PubMed-backed Aedes aegypti olfaction literature audit lane.")
