...
FATAL_REFRESH_GAP_REASONS = {
    "swd_ncbi_marker_review_search_failed",
    "swd_ncbi_marker_review_summary_failed",
}
...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_NCBI_MARKER_REVIEW_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    source_payload = {
        ...
        "marker_group_counts": result.marker_group_counts,
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_NCBI_MARKER_REVIEW_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_NCBI_MARKER_REVIEW_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_NCBI_MARKER_REVIEW_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_NCBI_MARKER_REVIEW_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_ncbi_marker_review(
    *,
    ...
    result = fetch_drosophila_suzukii_ncbi_marker_review_records(
        raw_dir=artifact_dir / "raw" / "drosophila_suzukii_ncbi_marker_review",
    ...
    if not refresh_failed:
        index.replace_source_records(DROSOPHILA_SUZUKII_NCBI_MARKER_REVIEW_SOURCE_ID, result.records)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest a broader NCBI marker-review lane for Drosophila suzukii.")
