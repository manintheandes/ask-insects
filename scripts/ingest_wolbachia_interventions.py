...
from askinsects.sources.wolbachia_interventions import (
    DEFAULT_WOLBACHIA_SOURCES,
    WOLBACHIA_INTERVENTION_SOURCE_ID,
    fetch_wolbachia_intervention_records,
)
...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    if not isinstance(existing, list):
        existing = []
    combined = [gap for gap in existing if not (isinstance(gap, dict) and gap.get("source") == WOLBACHIA_INTERVENTION_SOURCE_ID)]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[WOLBACHIA_INTERVENTION_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if WOLBACHIA_INTERVENTION_SOURCE_ID not in sources:
                sources.append(WOLBACHIA_INTERVENTION_SOURCE_ID)
        ...
        payload[WOLBACHIA_INTERVENTION_SOURCE_ID] = source_payload
...
def ingest_wolbachia_interventions(
    *,
    ...
    sources = list(DEFAULT_WOLBACHIA_SOURCES)
    if source_urls:
        sources = [
            {
                "organization": "World Mosquito Program",
                "url": url,
                "topic": "Aedes aegypti Wolbachia intervention evidence",
    ...
    result = fetch_wolbachia_intervention_records(
        sources,
        raw_dir=artifact_dir / "raw" / "wolbachia_interventions",
    ...
    outcome = run_source_ingest(
        ...
        persist_gap_records=False,  # gaps are plain dicts; non-gap records always present on success
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest World Mosquito Program Wolbachia intervention evidence into Ask Insects.")
    parser.add_argument("--artifact-dir", default=str(DEFAULT_ARTIFACT_DIR))
    parser.add_argument("--source-url", action="append", help="Override default Wolbachia evidence URLs. Can be passed multiple times.")
    parser.add_argument("--retrieved-at")
    args = parser.parse_args(argv)
    result = ingest_wolbachia_interventions(
