...
def _update_metadata(artifact_dir: Path, result) -> dict[str, object]:
    ...
    source_payload = {
        ...
        "method": "derived Aedes aegypti image atoms from indexed iNaturalist and Mosquito Alert still-image media payloads",
    }
...
def ingest_image_atoms(*, artifact_dir: Path = DEFAULT_ARTIFACT_DIR, retrieved_at: str | None = None) -> dict[str, object]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = ingest_image_atoms(artifact_dir=Path(args.artifact_dir), retrieved_at=args.retrieved_at)
