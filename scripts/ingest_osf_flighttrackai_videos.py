...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    if not isinstance(existing, list):
        existing = []
    combined = [gap for gap in existing if not (isinstance(gap, dict) and gap.get("source") == OSF_FLIGHTTRACKAI_SOURCE_ID)]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    source_payload = {
        ...
        "software_file_count": result.software_file_count,
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[OSF_FLIGHTTRACKAI_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if OSF_FLIGHTTRACKAI_SOURCE_ID not in sources:
                sources.append(OSF_FLIGHTTRACKAI_SOURCE_ID)
        ...
        payload["osf_flighttrackai_videos"] = source_payload
...
def ingest_osf_flighttrackai_videos(
    *,
    ...
    persist_source_gaps(index, OSF_FLIGHTTRACKAI_SOURCE_ID, result.gaps, retrieved_at=retrieved)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest OSF FlightTrackAI Aedes aegypti video/file manifests into Ask Insects.")
