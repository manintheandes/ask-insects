...
from askinsects.sources.drosophila_suzukii_olfaction_literature import (
    DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID,
    fetch_drosophila_suzukii_olfaction_literature_records,
)
...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    source_payload = {
        "source": DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID,
        "query": result.query,
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_olfaction_literature(
    *,
    ...
    result = fetch_drosophila_suzukii_olfaction_literature_records(
        raw_dir=artifact_dir / "raw" / "drosophila_suzukii_olfaction_literature",
    ...
    outcome = run_source_ingest(
        ...
        source_id=DROSOPHILA_SUZUKII_OLFACTION_LITERATURE_SOURCE_ID,
