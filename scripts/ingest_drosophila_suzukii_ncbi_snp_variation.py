...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_NCBI_SNP_VARIATION_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str) -> dict[str, object]:
    ...
    variant_record_count = len([record for record in result.records if not record.record_id.startswith("swd_ncbi_snp_variation:gap:")])
    source_payload = {
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_NCBI_SNP_VARIATION_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_NCBI_SNP_VARIATION_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_NCBI_SNP_VARIATION_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_NCBI_SNP_VARIATION_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_ncbi_snp_variation(
    *,
    ...
    fetch_json=None,
    fetch_records_fn=fetch_drosophila_suzukii_ncbi_snp_variation_records,
    retrieved_at: str | None = None,
) -> dict[str, object]:
    retrieved = retrieved_at or utc_now()
    result = fetch_records_fn(
        raw_dir=artifact_dir / "raw" / "drosophila_suzukii_ncbi_snp_variation",
        limit=limit,
        page_size=page_size,
    ...
    index = SourceIndex(artifact_dir / "source_index.sqlite")
    index.initialize()
    index.replace_source_records(DROSOPHILA_SUZUKII_NCBI_SNP_VARIATION_SOURCE_ID, result.records)
    return _update_metadata(artifact_dir, result, retrieved)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest an NCBI dbSNP variation audit for Drosophila suzukii.")
