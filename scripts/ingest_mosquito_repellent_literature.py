...
from askinsects.sources.mosquito_repellent_literature import (
    MOSQUITO_REPELLENT_LITERATURE_SOURCE_ID,
    fetch_mosquito_repellent_literature_records,
)
...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == MOSQUITO_REPELLENT_LITERATURE_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    source_payload = {
        ...
        "crossref_queries": list(result.crossref_queries),
        "requested_urls": result.requested_urls,
        "record_count": installed_record_count,
        "refresh_record_count": len(result.records),
        "pubmed_reported_total_count": result.pubmed_reported_total_count,
        "crossref_reported_total_count": result.crossref_reported_total_count,
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[MOSQUITO_REPELLENT_LITERATURE_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if MOSQUITO_REPELLENT_LITERATURE_SOURCE_ID not in sources:
                sources.append(MOSQUITO_REPELLENT_LITERATURE_SOURCE_ID)
        ...
        payload[MOSQUITO_REPELLENT_LITERATURE_SOURCE_ID] = source_payload
...
def ingest_mosquito_repellent_literature(
    *,
    ...
    result = fetch_mosquito_repellent_literature_records(
        raw_dir=artifact_dir / "raw" / "mosquito_repellent_literature",
    ...
    if not refresh_failed:
        index.replace_source_records(MOSQUITO_REPELLENT_LITERATURE_SOURCE_ID, result.records)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest mosquito repellent article metadata from PubMed and Crossref.")
    parser.add_argument("--artifact-dir", default=str(DEFAULT_ARTIFACT_DIR))
    parser.add_argument("--pubmed-max-results", type=int, default=1000)
    parser.add_argument("--crossref-max-results", type=int, default=1000)
