...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    if not isinstance(existing, list):
        existing = []
    combined = [gap for gap in existing if not (isinstance(gap, dict) and gap.get("source") == FIGSHARE_AEDES_VIDEO_SOURCE_ID)]
...
def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[FIGSHARE_AEDES_VIDEO_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if FIGSHARE_AEDES_VIDEO_SOURCE_ID not in sources:
                sources.append(FIGSHARE_AEDES_VIDEO_SOURCE_ID)
        ...
        payload["figshare_aedes_videos"] = source_payload
...
def ingest_figshare_aedes_videos(
    *,
    ...
    if not refresh_failed:
        index.replace_source_records(FIGSHARE_AEDES_VIDEO_SOURCE_ID, result.records)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest Figshare Aedes aegypti video search results into Ask Insects.")
