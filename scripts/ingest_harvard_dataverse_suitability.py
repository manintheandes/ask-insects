#!/usr/bin/env python3
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[1]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from askinsects.builder import DEFAULT_ARTIFACT_DIR, utc_now, write_json
from askinsects.index import SourceIndex
from askinsects.sources.harvard_dataverse_suitability import (
    DEFAULT_QUERIES,
    HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID,
    fetch_harvard_dataverse_suitability_records,
)


def _read_json(path: Path, default: object) -> object:
    if not path.exists():
        return default
    return json.loads(path.read_text(encoding="utf-8"))


def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    existing = _read_json(gaps_path, [])
    if not isinstance(existing, list):
        existing = []
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID)
    ]
    combined.extend(gaps)
    write_json(gaps_path, combined)
    return len(combined)


def _source_counts(index: SourceIndex) -> dict[str, int]:
    return {
        row["source"]: int(row["n"])
        for row in index.sql("select source, count(*) as n from records group by source order by source", limit=3000)
    }


def _source_count(index: SourceIndex) -> int:
    with index.connect() as conn:
        return int(
            conn.execute(
                "select count(*) as n from records where source=?",
                (HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID,),
            ).fetchone()["n"]
        )


def _update_metadata(artifact_dir: Path, result, retrieved_at: str, *, ok: bool = True, preserved_existing: bool = False) -> dict[str, object]:
    index = SourceIndex(artifact_dir / "source_index.sqlite")
    summary = index.summary()
    source_counts = _source_counts(index)
    source_payload = {
        "source": HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID,
        "record_count": len(result.records),
        "file_record_count": result.file_record_count,
        "query_count": result.query_count,
        "search_item_count": result.search_item_count,
        "dataset_count": result.dataset_count,
        "raw_artifacts": result.raw_artifacts,
        "gap_count": len(result.gaps),
        "retrieved_at": retrieved_at,
        "method": "bounded Harvard Dataverse file search and dataset-detail manifests for Aedes aegypti suitability rasters",
        "refresh_failed": not ok,
        "preserved_existing": preserved_existing,
    }
    gap_count = _append_dedup_gaps(artifact_dir / "gaps.json", result.gaps)
    for filename in ("source_status.json", "source_receipt.json"):
        path = artifact_dir / filename
        payload = _read_json(path, {})
        if not isinstance(payload, dict):
            payload = {}
        sources = payload.get("sources")
        if isinstance(sources, dict):
            sources[HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID not in sources:
                sources.append(HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID)
        payload["sources"] = sources
        payload["source_counts"] = source_counts
        payload["record_count"] = summary["record_count"]
        payload["species_count"] = summary["species_count"]
        payload["lanes"] = summary["lanes"]
        payload["gap_count"] = gap_count
        payload[HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID] = source_payload
        write_json(path, payload)
    return {
        "ok": ok,
        "source": HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID,
        "record_count": len(result.records),
        "preserved_existing": preserved_existing,
        "file_record_count": result.file_record_count,
        "gap_count": len(result.gaps),
        "query_count": result.query_count,
        "search_item_count": result.search_item_count,
        "dataset_count": result.dataset_count,
        "source_counts": source_counts,
        "artifact_dir": artifact_dir.as_posix(),
        "lanes": summary["lanes"],
    }


def ingest_harvard_dataverse_suitability(
    *,
    artifact_dir: Path = DEFAULT_ARTIFACT_DIR,
    queries: tuple[str, ...] = DEFAULT_QUERIES,
    per_page: int = 25,
    dataset_limit: int = 12,
    fetch_json=None,
    fetch_harvard_dataverse_suitability_records_fn=fetch_harvard_dataverse_suitability_records,
    retrieved_at: str | None = None,
) -> dict[str, object]:
    retrieved = retrieved_at or utc_now()
    result = fetch_harvard_dataverse_suitability_records_fn(
        raw_dir=artifact_dir / "raw" / "harvard_dataverse_suitability",
        queries=queries,
        per_page=per_page,
        dataset_limit=dataset_limit,
        fetch_json=fetch_json,
        retrieved_at=retrieved,
    )
    index = SourceIndex(artifact_dir / "source_index.sqlite")
    index.initialize()
    refresh_failed = not result.records and bool(result.gaps)
    if not refresh_failed:
        index.replace_source_records(HARVARD_DATAVERSE_SUITABILITY_SOURCE_ID, result.records)
    return _update_metadata(
        artifact_dir,
        result,
        retrieved,
        ok=not refresh_failed,
        preserved_existing=refresh_failed and _source_count(index) > 0,
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Ingest Harvard Dataverse Aedes aegypti suitability raster manifests.")
    parser.add_argument("--artifact-dir", default=str(DEFAULT_ARTIFACT_DIR))
    parser.add_argument("--query", action="append", default=[])
    parser.add_argument("--per-page", type=int, default=25)
    parser.add_argument("--dataset-limit", type=int, default=12)
    parser.add_argument("--retrieved-at")
    args = parser.parse_args(argv)
    result = ingest_harvard_dataverse_suitability(
        artifact_dir=Path(args.artifact_dir),
        queries=tuple(args.query) if args.query else DEFAULT_QUERIES,
        per_page=args.per_page,
        dataset_limit=args.dataset_limit,
        retrieved_at=args.retrieved_at,
    )
    print(json.dumps(result, sort_keys=True))
    return 0 if result.get("ok") else 2


if __name__ == "__main__":
    raise SystemExit(main())
