...
def _append_dedup_gaps(gaps_path: Path, gaps: list[dict[str, object]]) -> int:
    ...
    combined = [
        gap
        for gap in existing
        if not (isinstance(gap, dict) and gap.get("source") == DROSOPHILA_SUZUKII_SUSCEPTIBILITY_ASSAY_ROWS_SOURCE_ID)
    ]
...
def _update_metadata(artifact_dir: Path, result, *, ok: bool = True) -> dict[str, object]:
    ...
    source_payload = {
        ...
        "method": "schema-validated table-row and candidate susceptibility evidence promotion from drosophila_suzukii_extracted_facts resistance records",
    }
    ...
    for filename in ("source_status.json", "source_receipt.json"):
        ...
        if isinstance(sources, dict):
            sources[DROSOPHILA_SUZUKII_SUSCEPTIBILITY_ASSAY_ROWS_SOURCE_ID] = source_payload
        else:
            if not isinstance(sources, list):
                sources = []
            if DROSOPHILA_SUZUKII_SUSCEPTIBILITY_ASSAY_ROWS_SOURCE_ID not in sources:
                sources.append(DROSOPHILA_SUZUKII_SUSCEPTIBILITY_ASSAY_ROWS_SOURCE_ID)
        ...
        payload[DROSOPHILA_SUZUKII_SUSCEPTIBILITY_ASSAY_ROWS_SOURCE_ID] = source_payload
...
def ingest_drosophila_suzukii_susceptibility_assay_rows(
    *,
    artifact_dir: Path = DEFAULT_ARTIFACT_DIR,
    retrieved_at: str | None = None,
) -> dict[str, object]:
    result = build_drosophila_suzukii_susceptibility_assay_records(artifact_dir, retrieved_at=retrieved_at)
    index = SourceIndex(artifact_dir / "source_index.sqlite")
    index.initialize()
    refresh_failed = not result.records and bool(result.gaps)
    if not refresh_failed:
        index.replace_source_records(DROSOPHILA_SUZUKII_SUSCEPTIBILITY_ASSAY_ROWS_SOURCE_ID, result.records)
    return _update_metadata(artifact_dir, result, ok=not refresh_failed)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Promote Drosophila suzukii susceptibility evidence rows into Ask Insects.")
