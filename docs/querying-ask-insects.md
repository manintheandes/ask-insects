...
Dryad behavior/video records use source id `dryad_aedes_behavior_videos`. Raw dataset, version, and file-manifest JSON is saved under `artifacts/mosquito-v1/raw/dryad_behavior_videos/`. Dataset records use lane `behavior`; video/archive file records use lane `media`; README/source-data files use lane `behavior`; video/archive files also get `dryad_archive_contents_not_decoded` gap records until the archives are expanded into inspectable and motion-queryable rows. CSV/XLSX source-data files are bounded table-parse candidates; if the public download route blocks row parsing, query `show Dryad behavior table gaps` to see the exact files, sizes, checksums, download URLs, and errors. Each record stores the DOI, behavior labels, license, file size, checksum, download URL when present, raw manifest payload, and a provenance locator into the saved Dryad API JSON. The default ingest indexes metadata and download locators only; it does not mirror large video archives.
