...
The source index records raw SRA download inputs and a reanalysis workflow, but it does not claim the compute-heavy raw alignment/count outputs have already been generated. A complete public whole-brain connectome download is still an external availability gap.
