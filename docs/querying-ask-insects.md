...
This lane treats extension guidance as page-grain management evidence. It keeps the raw HTML locator and source URL for each page, but does not claim susceptibility assay rows, candidate biocontrol outcome rows, or human-validated biocontrol outcomes. Use `drosophila_suzukii_susceptibility_assay_rows` for SWD insecticide susceptibility/resistance evidence rows, and `drosophila_suzukii_biocontrol_outcome_rows` for SWD parasitoid and biological-control outcome evidence rows.
...
To ingest SWD biocontrol outcome evidence rows:

```bash
python3 -m askinsects --artifact-dir artifacts/mosquito-v1 ingest-drosophila-suzukii-biocontrol-outcome-rows
python3 -m askinsects --artifact-dir artifacts/mosquito-v1 ask "show Drosophila suzukii parasitoid biocontrol outcomes" --json
python3 -m askinsects --artifact-dir artifacts/mosquito-v1 sql "select json_extract(payload_json, '$.confidence') as confidence, count(*) as n from record_payloads where source='drosophila_suzukii_biocontrol_outcome_rows' group by confidence"
```
...
```bash
python3 -m askinsects --artifact-dir artifacts/mosquito-v1 ingest-drosophila-suzukii-plos-climate-suitability
python3 -m askinsects --artifact-dir artifacts/mosquito-v1 ask "show Drosophila suzukii climate suitability evidence" --json
python3 -m askinsects --artifact-dir artifacts/mosquito-v1 sql "select json_extract(payload_json, '$.atom_type') as atom_type, count(*) as n from record_payloads where source='drosophila_suzukii_plos_climate_suitability' group by atom_type"
```

This lane preserves the PLOS article model summary, supplement DOCX/XLSX file manifests, row-level PCA/correlation/Moran's I table facts, checksums, byte sizes, and an explicit gap for raw suitability raster grids that are not downloadable from the article package.
...
The same SWD video-atoms lane also maps Dryad `10.5061/dryad.8vd762q`, which is not ordinary MP4 video. It is SWD-involved copulation evidence stored as large ZIP archives of 5 fps TIFF frame sequences. The ingest records the five SWD-involved frame archives as queryable media atoms. With `--mirror-videos --generate-artifacts`, under-cap CC0 archives are mirrored, SHA-256 and MD5 checksummed, compared with source-provided Dryad MD5 when present, expanded into ZIP member rows and frame manifests, and sampled into keyframe/thumbnail artifacts when TIFF decoding is available. It keeps explicit gaps when Dryad preview/download routes are blocked, an archive is too large to mirror, TIFF frames are not decoded to video, inner spreadsheets are not parsed, or motion rows are not available.
...
Mendeley behavior/media records use source id `mendeley_aedes_behavior_media`. Raw public snapshot, folder, and file manifests are saved under `artifacts/mosquito-v1/raw/mendeley_behavior_media/`. Each source-provided audio file has a behavior record that keeps the file locator, checksum, byte size, license, download URL, folder path, and folder-derived acoustic context such as `665 Hz vs white noise`; bounded public WAV files also get decoded metadata records with duration, sample rate, channel count, sample width, frame count, byte rate, and raw audio locator. These are file-level audio facts, not full acoustic feature extraction. Parsed table rows include row-level species when a table names a non-aegypti Aedes species, plus typed behavior labels such as trajectory, locomotory assay, acoustic wingbeat, phonotaxis, or electrophysiology. Ask questions such as `show Mendeley Aedes wingbeat acoustic audio metadata`, `show Mendeley Aedes trajectory rows`, or `what Mendeley table rows mention temperature gradients?`.
...
Dryad behavior/video records use source id `dryad_aedes_behavior_videos`. Raw dataset, version, file-manifest JSON, and public landing-page HTML are saved under `artifacts/mosquito-v1/raw/dryad_behavior_videos/`. Dataset records use lane `behavior`; video/archive file records use lane `media`; README/source-data files and landing-page assay-method sections use lane `behavior`; video/archive files also get `dryad_archive_contents_not_decoded` gap records until the archives are expanded into inspectable and motion-queryable rows. CSV/XLSX source-data files are bounded table-parse candidates; if the public download route blocks row parsing, query `show Dryad behavior table gaps` to see the exact files, sizes, checksums, download URLs, and errors. Each record stores the DOI, behavior labels, license, file size, checksum, API download URL, browser-facing file-stream URL when derivable, raw manifest payload, and a provenance locator into saved Dryad API JSON or landing-page HTML. The default ingest indexes metadata, public assay descriptions, and download locators only; it does not mirror large video archives.
...
This lane emits supplement manifests plus deterministic candidate facts for vector competence, resistance, behavior, ecology, and public health. With opt-in supplement discovery and download, it also parses supported `.csv`, `.tsv`, `.xlsx`, `.docx`, XML table, and simple HTML table rows into `parsed` fact records with raw-file and row provenance. Metadata discovery covers record-payload links, identifier-backed Europe PMC, PMC OA files, Crossref relations, DataCite related identifiers, Unpaywall OA locations, Figshare, Zenodo, Dryad, NCBI BioProject metadata, NCBI GEO metadata, ProteomeXchange/PRIDE metadata, protocols.io public protocol metadata, GitHub public repository metadata recovered from publisher XML links, publisher landing-page links, and supplement-looking URLs inside legal full-text units. It is separately bounded by `--max-supplement-discovery-records` so production runs do not make unbounded literature-wide lookups, and receipts expose `supplement_discovery_route_counts` for route-level proof. It also emits one `supplement_audit` literature record per indexed Aedes paper, so supplement coverage can be counted by paper instead of by lucky manifest hits. `no_supplement_metadata_found` means no configured bounded route found public supplement metadata in that run; it is not proof that no supplement exists anywhere. It is not yet human-validated extraction.
