...
Mendeley behavior/media records use source id `mendeley_aedes_behavior_media`. Raw public snapshot, folder, and file manifests are saved under `artifacts/mosquito-v1/raw/mendeley_behavior_media/`. Each source-provided audio file now has a behavior record that keeps the file locator, checksum, byte size, license, download URL, folder path, and folder-derived acoustic context such as `665 Hz vs white noise`; these records are manifest/context evidence and do not claim decoded waveform features. Parsed table rows include row-level species when a table names a non-aegypti Aedes species, plus typed behavior labels such as trajectory, locomotory assay, acoustic wingbeat, phonotaxis, or electrophysiology. Ask questions such as `show Mendeley Aedes wingbeat acoustic audio files`, `show Mendeley Aedes trajectory rows`, or `what Mendeley table rows mention temperature gradients?`.
