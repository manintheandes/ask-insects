...
The Mendeley behavior/media ingest saves one raw snapshot JSON, one folder JSON, one combined folder-file manifest JSON per dataset, and public parsed table files under `raw/mendeley_behavior_media/`. It indexes one `behavior` record per dataset, one `behavior` record per folder, file-level `media` records for video, audio, or archive files, file-level `behavior` records for spreadsheet, source-data, README, and code files, acoustic behavior records for source-provided sound files, parsed sheet records, and parsed table-row records. It preserves DOI, license, folder path, size, content type, SHA-256 hash, download URL, view URL, behavior labels, audio folder-context labels, table behavior type, row-level species when a table names a comparison species, headers, row values, and raw locators without mirroring large binaries by default.
