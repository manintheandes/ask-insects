...
`drosophila_suzukii_jki_drosomon_trap_captures` adds source-grade field-monitoring evidence for spotted wing drosophila. It indexes the data.europa/OpenAgrar registry record for the 7-year JKI DrosoMon monitoring dataset in southwest Germany, the `captures_data.csv` and parameter-description file manifests, article DOI `10.3390/insects9040125`, CC-BY-4.0 license, reported 100 traps, 9,967 trap-deployment records, 116,602 trap-days, and 756,717 captured adults. Because the OpenAgrar CSV URL currently returns an HTML security-check page instead of CSV, the lane stores queryable source gaps for the blocked download and for deployment rows not yet being atomically queryable.
