...
8. Add the follow-on table deep parse for public Aedes `.csv`, `.tsv`, and `.xlsx` files.
9. Update README, source lanes, source map, coverage ledger, and completion gate.
10. Run targeted tests, full unit tests, and `python3 scripts/verify_complete.py`.
11. Run local ingest, deploy server, run hosted ingest, verify hosted counts and sample answers.
